...
describe('The license path is provided but does not exist', function () {
    ...
    it("should fail due to license file not found", {timeout:5000}, async() => {
        await tr.runAsync();
        assert.equal(tr.failed, true, 'The task should have failed');
        assert.equal(tr.errorIssues.length, 1, "should have 1 error issue");
        assert.equal(tr.errorIssues[0], "Not found /invalid/path/license.json", "Error message should include that the sbom path was not found");
